...
function generateWishes(event) {
  ...
  } wishes for a ${forSelect.value} set tone of the wishes to ${
    toneSelect.value
  } write wishes are dedicated to ${nameInput.value.trim().toUpperCase()}`;
  let context =
    "You are greetings and occasions wishes expert and love to write short beautiful wishes. Your mission is to write short 4 lines wishes in basic HTML. Please always seperate each sentence to new line. Make sure to follow user selected and inputed instructions. Do not include a title to the wishes. Sign the wishes </br> </br> <strong>SheCodes AI</strong> at the end of the wishes.";
  let apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  let wishesElement = document.querySelector("#wishes");
  wishesElement.classList.remove("hidden");
  wishesElement.innerHTML = `<div id="loading"></div>  Generating ${occasionSelect.value} wishes `;
